...
$.fn.extend({reference:function(b,m,g,e){var l=((e&&e.bindName)?e.bindName:"dblclick");if(arguments.length==0){this.trigger(l);return}var d="reference";var f=function(o,p,q){$.ajax({type:"post",url:($.isFunction(b)?b.call(this):b),dataType:"json",data:{keyword:o||"",length:e.len,page:p},success:function(s,t,r){if($.isNotEmpty(s.id)||$.isNotEmpty(s.main)){q.call(this,s)}else{alert(s.detailMessages.join("\n"))}}})};var a=function(r,s,p){r.push("<div class='total'><ul>");if($.isEmpty(p)){r.push("<li class='label'>全 "+s.records.length+" 件</li>")}else{var t=k(p,0);r.push("<li class='label'>"+t.startCount+" - "+t.endCount+" 件 (全 "+t.totalCount+" 件)</li>");var u=t.page-t.range;if(u<=0){u=1}var o=t.page+t.range;if(o>t.maxPage){o=t.maxPage}if(t.page>1){r.push("<li page='"+(t.page-1)+"'>前へ</li>")}if(u>1){r.push("<li>...</li>")}for(var q=u;q<=o;q++){if(q==t.page){r.push("<li class='current'>"+q+"</li>")}else{r.push("<li page='"+q+"'>"+q+"</li>")}}if(o<t.maxPage){r.push("<li>...</li>")}if(t.page<t.maxPage){r.push("<li page='"+(t.page+1)+"'><a>次へ</a></li>")}}r.push("</ul></div><br style='clear:both;'/>")};var k=function(u,q){var t={};var p=u.records[q];for(var r=0,o=u.fields,s=o.length;r<s;r++){t[o[r].name]=p[r]}return t};var c=function(s){var q={};for(var p=0,o=s.fields,r=o.length;p<r;p++){q[o[p].name]={field:o[p],index:p}}return q};var j=function(o,p,r){var q=p[r];if(q){return o[q.index]}return null};var i=function(q,p,r,o){r.find("tr").bind("mouseover",function(){$(this).toggleClass("over")}).bind("mouseout",function(){$(this).toggleClass("over")}).bind("click",function(){m.call(q,o[$(this).attr("index")]);p.dialog("close")});p.find("li[page]").click(function(){var t=$(this).attr("page");var s=p.find("[name=keyword]").val();h(false,s,t)})};var n=this;var h=function(q,o,p){f(o,p,function(M){var K=M.pagination;M=M.main||M;var A=$("#"+d);if(A.size()==0){A=$(document.body).append("<div id='"+d+"' class='reference'/>").find("#"+d)}A.unbind();var x=[];var D=65;var B=470;if($.isNotEmpty(e)&&e.search!=false){x.push("<div class='condition'>");x.push("	<form>");x.push("		<input type='text' name='keyword' value='"+o+"'/>");x.push("		<input type='submit' value='検索'/>");x.push("	</form>");x.push("</div>")}else{B-=30}a(x,M,K);x.push("<table cellspacing='0' cellpadding='0'>");x.push("<tr>");var u=c(M);for(var H=0,t=g.length;H<t;H++){var F=g[H];var z=u[F.name];if($.isNotEmpty(z)&&!F.hidden){var L=z.field.caption||F.name;x.push("<th width='"+F.width+"'>"+L+"</th>");D+=F.width}}x.push("</tr>");x.push("</table>");A.html(x.join(""));var w=A.append("<div class='records'><table id='records' cellspacing='0' cellpadding='0'></table></div>").find("#records");var x=[];var C=[];var I=g.length;var s=e.escape!=false;for(var H=0,J=M.records,t=J.length;H<t;H++){var r=J[H];var y=[];x.push("<tr index='"+H+"'>");for(var G=0;G<I;G++){var F=g[G];var E=j(r,u,F.name);if(!F.hidden){x.push("<td width='"+(F.width)+"'>"+((s)?$.escapeHtml(E):E)+"</td>")}y[G]=E}x.push("</tr>");C.push(y)}i(n,A,w.append(x.join("")),C);if(q){A.dialog($.extend((e||{}),{width:D,height:B,resizable:false,modal:true}))}var v=A.find("[name=keyword]").val(o);A.find("form").on("submit",function(){h(false,v.val());return false})})};this.bind(l,function(){h(true,null)});return this}});
